...
export function buildReviewTree(records: SourceRecord[], settings: ReviewCenterSettings, mode: ReviewMode): ReviewTreeNode[] {
  return groupsFor(settings, mode).map((group) => {
    ...
    const actual = records.filter((r) => r.sourceStatus !== "deleted" && resolveGroup(r.tags, groupsFor(settings, mode), r.sourcePath)?.id === group.id).flatMap((r) => normalizeTags(r.tags));
    if (!prefixes.length) prefixes.push(...[...new Set(actual.map((tag) => tag.split("/")[0]))].sort(naturalCompare));
